...
final class Strings {
	...
	public static function excerpt(string $str, int $length = 200, int $flags = 5) : string {
		$str = self::whitespace($str, 0, ($flags & globals_get("flag_trusted")));
		$str = strip_tags($str);

		$flagsEllipsis = !! ($flags & globals_get("flag_excerpt_ellipsis"));
		$flagsWord = !! ($flags & globals_get("flag_excerpt_break_word"));
		$flagsChar = !$flagsWord;
		$flagsInclusive = $flagsEllipsis && $flagsChar && ($flags & globals_get("flag_excerpt_inclusive"));
		...
		if ($flagsChar && mb_strlen($str, "UTF-8") > $length) {
			$str = (string) trim(mb_substr($str, 0, $length, "UTF-8"));
			if ($flagsEllipsis) {
				$str .= "…";
			}
		}
	...
	public static function niceText(string $str, int $newlines=0, int $flags=0) : string {
		$str = self::printable($str, ($flags & globals_get("flag_trusted")));
		$str = self::controlChars($str, globals_get("flag_trusted"));
		$str = self::quotes($str, globals_get("flag_trusted"));
		$str = self::whitespace($str, $newlines, globals_get("flag_trusted"));
	...
	public static function printable(string $str, int $flags=0) : string {
		...
		$str = preg_replace_callback(
			"/[^[:print:]]/u",
			[static::class, "printableCallback"],
			$str
		);
	...
	public static function toSentence(string $str, int $flags=0) : string {
		...
		if ($str) {
			if (!mb_check_encoding($str, "ASCII")) {
				...
				$first = self::toUpper($first, globals_get("flag_trusted"));
	...
	public static function utf8(string $str) : string {
		...
		while ($x < $length) {
			...
			if ($c1 >= $xc0) {
				$c2 = ($x + 1) >= $length ? strval($x00) : strval($str[$x + 1]);
				$c3 = ($x + 2) >= $length ? strval($x00) : strval($str[$x + 2]);
				$c4 = ($x + 3) >= $length ? strval($x00) : strval($str[$x + 3]);
				...
				elseif (($c1 >= $xe0) & ($c1 <= $xef)) {
					...
					// Invalid; convert it.
						...
						$cc1 = strval((chr(ord($c1) / 64) | $xc0));
						$cc2 = strval(($c1 & $x3f) | $x80);
						$out .= $cc1 . $cc2;
					}
				}
	...
	public static function whitespace(string $str, int $newlines=0, int $flags=0) : string {
		...
		foreach ($lines as $k=>$v) {
			$lines[$k] = trim(preg_replace("/\s+/u", " ", v));
		}
		$str = (string) implode("\n", $lines);
		$str = self::trim($str, globals_get("flag_trusted"));
	...
	public static function wordwrap(string $str, int $width=75, string $eol="\n", bool $cut=false) : string {
		...
		foreach ($chunks as $v) {
			...
			$v = self::trim($v, globals_get("flag_trusted"));
		...
		foreach ($lines as $v) {
			...
			if (!empty($preg_eol) && preg_match("/\v/u", $v)) {
				...
				elseif (1 === $breakLength) {
					$out[] = self::trim(ltrim($v, $eol), globals_get("flag_trusted"));
				}
				else {
			...
			$out[] = self::trim($v, globals_get("flag_trusted"));
		}
