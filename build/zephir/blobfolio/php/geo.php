...
final class Geo {
	...
	public static function niceAddress(array $parts, int $flags=7) : array {
		...
		$flagsEmail = !! ($flags & globals_get("flag_address_field_email"));
		$flagsPhone = !! ($flags & globals_get("flag_address_field_phone"));
		$flagsCompany = !! ($flags & globals_get("flag_address_field_company"));
		...
		if (!isset($parts["street"]) || empty($parts["street"])) {
			...
			foreach ($aliases as $v) {
				if (
					...
					$parts["street"] = trim($parts["street"]);
					break;
				}
				elseif (
	...
	public static function niceCountry(string $country) : string {
		...
		// Uppercase it.
		$country = \Blobfolio\Strings::toUpper($country, globals_get("flag_trusted"));
		...
		foreach (self::$_countries as $k=>$v) {
			$v["name"] = (string) \Blobfolio\Strings::toUpper($v["name"], globals_get("flag_trusted"));
			if ($country === $v["name"]) {
				return (string) $k;
			}
		}
	...
	public static function getUsStates(int $flags=1) : array {
		...
		$extra = !! (flags & globals_get("flag_us_territories"));
		if (!$extra) {
			$out = (array) self::$_us;
			unset($out["AA"]);
			unset($out["AE"]);
