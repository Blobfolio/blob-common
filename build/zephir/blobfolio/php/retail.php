...
final class Retail {
	...
	public static function usd($value, int $flags=1) : string {
		$money = (float) \Blobfolio\Cast::toFloat($value, globals_get("flag_flatten"));
		$money = round($money, 2);
		...
		// Parse flags.
		$flagsThousands = !! ($flags & globals_get("flag_usd_thousands"));
		$flagsCents = !! ($flags & globals_get("flag_usd_cents"));
		$flagsTrim = !! ($flags & globals_get("flag_usd_trim"));
		...
		if ($flagsCents && $money < 1) {
			...
			$out .= "$" . number_format($money, 2, ".", $separator);
	...
	public static function niceEan(string $str, int $flags=0) : string {
		...
		// Last thing, format?
		$formatted = !! ($flags & globals_get("flag_pretty"));
		if ($formatted) {
	...
	public static function niceName(string $str, int $flags=0) : string {
		$str = \Blobfolio\Strings::niceText($str, 0, ($flags & globals_get("flag_trusted")));
	...
	public static function splitName(string $str, int $flags=0) : array {
		$str = self::niceName($str, ($flags & globals_get("flag_trusted")));
		...
		if (false !== $start) {
			$out["firstname"] = trim(mb_substr($str, 0, $start, "UTF-8"));
			$out["lastname"] = trim(mb_substr($str, $start, null, "UTF-8"));
		}
	...
	public static function nicePassword(string $str, int $flags=0) : string {
		$str = \Blobfolio\Strings::printable($str, ($flags & globals_get("flag_trusted")));
